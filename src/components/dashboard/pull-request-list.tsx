...
import { useGitHubPulls } from "@/hooks/use-github";
...
function getAvatarInitial(author: string, avatarUrl: string): string {
  if (avatarUrl) return "";
...
export function PullRequestList() {
  const { t } = useLanguage();
  const { data: prs, loading, error, refresh } = useGitHubPulls();

  const isDemo = !loading && !error && Array.isArray(prs) && prs.some((p) => p.url === "#");
    ...
    <div className="card p-5 sm:p-6 space-y-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-[var(--text-tertiary)] mt-0.5 truncate">
              {loading ? "Loading..." : `${prs?.length ?? 0} open pull requests`}
            </p>
          </div>
        </div>
      ...
      {loading && (
        <div className="space-y-2.5">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.018)", border: "1px solid rgba(255,255,255,0.05)" }}>
      ...
      {!loading && !error && prs && (
        <div className="space-y-2.5">
          {prs.map((pr, idx) => {
            ...
            const initial = getAvatarInitial(pr.author, pr.avatarUrl);
            return (
              <a
                key={pr.id}
                href={pr.url !== "#" ? pr.url : undefined}
                target={pr.url !== "#" ? "_blank" : undefined}
                rel="noopener noreferrer"
                className="block p-4 rounded-xl transition-all duration-150 cursor-pointer group"
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    {pr.avatarUrl ? (
                      <img
                        src={pr.avatarUrl}
                        alt={pr.author}
                        className="w-8 h-8 rounded-lg shrink-0 mt-0.5 object-cover"
                      />
                    ) : (
                      <div
                    ...
                    <div className="space-y-1.5 flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-[var(--text-tertiary)] flex-wrap">
                        ...
                        <span>by <strong className="text-[var(--text-secondary)] font-semibold">@{pr.author}</strong></span>
                  ...
                  <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0 pl-11 lg:pl-0">
                    ...
                    >
                      <span className="flex items-center gap-0.5 text-[#10d98e]">
                        <Plus className="w-2.5 h-2.5" strokeWidth={2.5} />{pr.additions}
                      </span>
                      <span className="text-[var(--text-disabled)]">/</span>
                      <span className="flex items-center gap-0.5 text-[#fb7185]">
                        <Minus className="w-2.5 h-2.5" strokeWidth={2.5} />{pr.deletions}
                      </span>
                    </div>
